...
def process_file_with_sessions(file_path, output_filename, exchange_rate):
    ...
    try:
        ...
        for i in range(2, len(raw_df.columns)):
            ...
            # If the date is valid (not NaN) and the metric is not 'Unnamed' or 'nan'
            if pd.notna(date_val) and isinstance(date_val, pd.Timestamp) and 'Unnamed' not in metric_val and metric_val != 'nan':
                date_str = date_val.strftime('%Y-%m-%d')
                col_name = f"{date_str}_{metric_val}"
                new_columns.append(col_name)
                date_metric_columns_to_melt.append(col_name)
            elif 'Unnamed' not in metric_val and metric_val != 'nan':
                # This handles cases where the first row might be NaN but the second row has a valid metric
                # This is likely for the "Total" columns at the end
                new_columns.append(metric_val)
            else:
                # Placeholder for columns that couldn't be parsed
        ...
        # This is crucial because raw_df.columns might be longer than df.columns if there are empty columns at the end
